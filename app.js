...
app.post("/upload2", async (req, res) => {
  ...
  try {
    ...
    const uploadResult = await cloudinary.uploader.upload(images.tempFilePath, {
      folder: "user_uploads",
    });

    const uploadedImageUrl = uploadResult.secure_url; // Cloudinary URL of uploaded image
    ...
    // Step 3: Iterate through each image stored for the user and compare
    for (const storedImage of user.images) {
      ...
      // Fetch the uploaded image from Cloudinary using axios
      const uploadedImageData = await axios
        .get(uploadedImageUrl, { responseType: "arraybuffer" })
        .then((response) => Buffer.from(response.data)); // Convert to buffer

      // Prepare the Rekognition parameters
      const params = {
        SourceImage: {
          Bytes: targetImageData, // Stored image
        },
        TargetImage: {
          Bytes: uploadedImageData, // Uploaded image
        },
        SimilarityThreshold: 90,
      };

      // Step 4: Compare the faces using AWS Rekognition
    ...
    // Step 5: Return the result based on face match
    if (faceMatched) {
      res.json({
        success: true,
        message: "Face recognized!",
        data: { userface: uploadedImageUrl, user },
      });
    } else {
...
//   // Log req.files to check the structure and content of the uploaded file
